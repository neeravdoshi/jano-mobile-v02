...
| 2026-06-19 | V2 chat backdrop: faint Lucide-derived medical-glyph motif (`src/assets/chat-motif.svg`, ~5% opacity) tiled behind the transcript, fixed so messages scroll over it. NOTE: Vite inlines small SVGs as `data:` URIs containing parens + single quotes — CSS `url()` must be **double-quoted** (`url("${motif}")`) or the browser silently drops it to `none` |
